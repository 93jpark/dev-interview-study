...
> - 작은 클래스들의 **상속**과 **합성**을 이용하여 더 큰 클래스를 생성하는 방식
> - 서로 독립적으로 개발한 클래스 라이브러리를 하나인 양 사용 가능
> - 서로 다른 인터페이스들의 통일된 추상을 제공
...
- 복합 객체를 사용하고 있는지, 단일 객체를 사용하고 있는지에 대해서 신경 쓰지 않아도 되므로 **클라이언트를 단순화**시킬 수 있다

- 복합 객체를 하나의 단일화된 객체처럼 취급 할 수 있으므로 **일괄적인 관리(반복 잡업 등)** 시 편리하다
> **예시**<br>파워포인트에서 삼각형, 사각형, 원을 그리고 삼각형과 원을 그룹화 했다고 가정하자. 이때 우리는 모든 도형을 노란색으로 칠하려고 한다.
<br>그룹화한 도형에 채우기 버튼을 누를 때 선택하는 도형이 어떤 모양인지, 그룹인지에 관해 구분하지 않아도 된다. 파워포인트에는 개별 도형과 그룹 전체에 대한 채우기 버튼이 같다.
><Br> 이와 같이 복합체 패턴은 전체 도형들을 개별 도형을 다루듯이 관리할 수 있다는 특징이 있다. **일괄적인 관리**가 가능하다고 할 수 있는 것이다.
